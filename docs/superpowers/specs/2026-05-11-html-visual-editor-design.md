...
本工具编辑的是浏览器内存中的页面副本，不会修改任何远程服务器上的原始文件。对在线网页的所有修改仅存在于用户本地，刷新页面即恢复原状。用户可通过导出功能将修改后的副本保存为本地 HTML 文件。

首次进入编辑模式时，工具栏区域应显示一条提示："您正在编辑本地副本，不会影响原始网站"，用户可关闭此提示，后续不再重复显示。
...
5. 首次使用时显示提示："您正在编辑本地副本，不会影响原始网站"
6. 页面进入可编辑状态
